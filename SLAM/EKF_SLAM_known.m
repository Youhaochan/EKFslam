...
for i = start:size(Robots{robot_num}.G, 1)
    ...
    if z(3,1) > 1
        ...
        for k = 1:size(z, 2) 
            j = z(3,k);
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % 只有这里是和普通的ekf有所不同的,其他的地方都是一样的, ekf slam如果是假设landmark的位置不知道
            % 然后进行估计landmark的位置
            % if the landmark has never been seen before
            % add it to the state vector
            if stateMeanBar(j*2 + 2) == 0
                % landmark_pos_x = mean_x +  r * cos(phi + omega)
                % landmark_pos_y = mean_y +  r * sin(phi + omega)
                % 上面两个公式是用来估计landmark的位置的，其中r为传感器测出的 range
                % phi 为传感器测出的bearing， mean_x， mean_y 和 omega是predictin的均值
                % 和角度 也就是predict 的机器人的位姿
            ...
            %如果发现当前landmark已经有3d位置了,则直接和普通ekf一样的进行update
